...
def get_medical_vo2max(age, gender, user_healthscore):
	...
	healthscore_range = plot_healthscore.plot_healthscore(df[(df['gender']==gender) & (df['Age']==med_age_group)], user_healthscore)	
	...
	for ind, el in enumerate(health_indicator.keys()):
		if ind == 0 and user_healthscore < el:
			return [health_indicator[el], healthscore_range]
			...
			return [health_indicator[list(health_indicator.keys())[ind-1]], healthscore_range]	
...
def get_medical_sleep(age, user_sleepscore):
	...
	sleepscore_range = plot_sleepscore.plot_sleepscore(sleep_range, user_sleepscore)

	if user_sleepscore < sleep_range[0]:
		return ('Poor', sleep_range, sleepscore_range)
	else:
		return ('Good', sleep_range, sleepscore_range)
...
def get_medical(age, gender, user_healthscore, user_sleepscore):
	...
	medical['sleep_tag'], medical['sleep'], medical['plot'] = get_medical_sleep(age, user_sleepscore)
